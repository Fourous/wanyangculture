...
function judgeRorL($uid,$bookid){
	global $db;
	$sql = "select * from lend_record where uid = '{$uid}' and bookid = '{$bookid}' and ispay=0;";
	$row_count = $db->query($sql);
	if($row_count){
...
function handleOrder($order,$boxid)
{
	...
	for($i=0;$i<$booknumber;$i++){
		...
		else lendBook($uid,$book[$i],$boxsid,$Gid);
	}
}
	}
