...
class WeixinPay {
    ...
    protected $total_fee;
	protected $remote_ip;
            function __construct($appid, $openid, $mch_id, $key,$out_trade_no,$body,$total_fee,$remote_ip) {
        $this->appid = $appid;
        $this->openid = $openid;
        $this->mch_id = $mch_id;
        $this->key = $key;
...
//            'spbill_create_ip' => $_SERVER['REMOTE_ADDR'], //终端IP
            'spbill_create_ip' => $this->remote_ip, //终端IP
            'notify_url' => 'https://www.wanyangculture/php/pay/notify.php', //通知地址  确保外网能正常访问
...
//        print_r($unifiedorder);
		if($unifiedorder['return_code']=='FAIL'){
			$returnInfo['code'] = 0;
			$returnInfo['msg'] = $unifiedorder['return_msg'];
			return $returnInfo;
		}
        $parameters = array(
            'appId' => $this->appid, //小程序ID
